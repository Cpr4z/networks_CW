...
void NoteClient::handleUpdateTextMergedRequest(const std::vector<uint8_t>& buffer) {
    ...
    if (request_opt.has_value()) {
        const auto& request = request_opt.value();
//        std::cout << "Update text merged request data:" << std::endl;
//        std::cout << "Note id is: " << request.note_id << std::endl;
//        std::cout << "Note version is: " << request.version << std::endl;
//        std::cout << "Note merged_version text is: " << request.merged_version << std::endl;
...
//                    std::cout << "Got server approve merge request" << std::endl;
                    handleServerApproveMergeRequest(buffer);
                    break;

                case Protocol::Operation::UPDATE_TEXT_MERGED:
//                    std::cout << "Got update text merged request" << std::endl;
                    handleUpdateTextMergedRequest(buffer);
                    break;
                default:
                    std::cerr << "Unknown operation code: " << static_cast<uint16_t>(op) << std::endl;
                    break;
            }
        }
    }
